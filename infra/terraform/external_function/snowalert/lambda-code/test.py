...
def test_nexus_repos():
    result = lambda_handler(
        {
            "path": "/https",
            "headers": {
                "sf-custom-base_url": "https://nexus.int.snowflakecomputing.com",
                "sf-custom-url": "/service/rest/v1/{0}",
                "sf-custom-cursor": "continuationToken",
                "sf-custom-params": "",
                "sf-custom-results-path": "items",
            },
            "body": '{"data": [[0, "repositories"]]}',
        }
    )
    assert len(loads(result["body"])["data"][0]) == 1
    ...
    result = lambda_handler(
        {
            ...
            "headers": {
                "sf-custom-base_url": "https://nexus.int.snowflakecomputing.com",
                "sf-custom-url": "/service/rest/v1/components",
                "sf-custom-cursor": "continuationToken",
                "sf-custom-params": urlencode({"repository": "sf_npm_repo"}),
                "sf-custom-results-path": "items",
            },
            "body": '{"data": [[0, "components", "respository", "sf_npm_repo"]]}',
        }
    )
    assert len(loads(result["body"])["data"][0][1]) >= 10
