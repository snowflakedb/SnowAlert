...
```sql
ALTER TABLE azure_collect_pricings ADD COLUMN raw VARIANT;
```

# Update Azure Collect Users table to add raw col

```sql
ALTER TABLE azure_collect_users ADD COLUMN raw VARIANT;
```
...
# Update Azure Inventory to add public IP addresses table

```sql
CREATE OR REPLACE TABLE data.azure_collect_public_ip_addresses (
    recorded_at TIMESTAMP_LTZ,
    tenant_id VARCHAR(50),
    subscription_id VARCHAR(50),
    error VARIANT,
...
GRANT SELECT, INSERT
ON data.azure_collect_public_ip_addresses
TO ROLE snowalert;
```
...
## Get AWS User Inline Policy Documents
```sql
CREATE OR REPLACE TABLE data.aws_collect_iam_get_user_policy (
    recorded_at TIMESTAMP_LTZ,
    account_id STRING,
    policy_name STRING,
    user_name STRING,
...
;
GRANT SELECT, INSERT
ON data.aws_collect_iam_get_user_policy
TO ROLE snowalert
;
```

## Get AWS Group Inline Policy List
```sql
CREATE OR REPLACE TABLE data.aws_collect_iam_list_group_policies (
    recorded_at TIMESTAMP_LTZ,
    account_id STRING,
    group_name STRING,
    error VARIANT,
...
;
GRANT SELECT, INSERT
ON data.aws_collect_iam_list_group_policies
TO ROLE snowalert
;
```

## Get AWS Group Inline Policy Documents
```sql
CREATE OR REPLACE TABLE data.aws_collect_iam_get_group_policy (
    recorded_at TIMESTAMP_LTZ,
    account_id STRING,
    policy_name STRING,
    group_name STRING,
...
;
GRANT SELECT, INSERT
ON data.aws_collect_iam_get_group_policy
TO ROLE snowalert
;
```

## Get AWS Role Managed Policies
```sql
CREATE OR REPLACE TABLE data.aws_collect_iam_list_attached_role_policies (
    recorded_at TIMESTAMP_LTZ,
    account_id STRING,
    role_name STRING,
    error VARIANT,
...
;
GRANT SELECT, INSERT
ON data.aws_collect_iam_list_attached_role_policies
TO ROLE snowalert
;
```
