...
~~~
ALTER TABLE results.alerts ADD COLUMN id VARCHAR(50);
~~~

## Update Azure and AWS Inventory and Configuration Connectors

The new version of the connector adds columns for the most recent Azure Cloud
changes. If you have a deployed default connector, you can update it with:
...
The new version of the connectors records errors in responses from Azure. You
can add the columns to a deployed default connector with:
...
The new version of the connector updates the following columns for consistency
and removes vestigial collumns:
...
The new version of the connector collects results of az commands `list groups`,
`groups members`, and aws commands `list attached group policies`, `role
assignments`. To add landing tables for these to a default connector, you can
run:
