...
testAlert = {  'TITLE': 'test SnowAlert'
             , 'DETECTOR': 'SnowAlert'
             , 'SEVERITY': 'info'
             , 'DETECTOR': 'SnowAlert'
...
def handle(
    ...
    if severity not in severityDictionary:
        log.warn( f"Set severity to {severityDictionary[-1]}, supplied {severity} is not in allowed values: {severityDictionary}" )
        severity = severityDictionary[-1]

    custom_details = custom_details or alert

    try:
        response = pds.trigger( summary, source, dedup_key, severity, custom_details=alert )
