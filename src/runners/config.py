...
RULES_SCHEMA = environ.get(
    'SA_RULES_SCHEMA',
    f"{DATABASE}.{RULES_SCHEMA_NAME}"
)
RESULTS_SCHEMA = environ.get(
    'SA_RESULTS_SCHEMA',
    f"{DATABASE}.{RESULTS_SCHEMA_NAME}"
)

# tables
ALERTS_TABLE = environ.get(
    'SA_ALERTS_TABLE',
    f"{RESULTS_SCHEMA}.{RESULTS_ALERTS_TABLE_NAME}"
)
VIOLATIONS_TABLE = environ.get(
    'SA_VIOLATIONS_TABLE',
    f"{RESULTS_SCHEMA}.{RESULTS_VIOLATIONS_TABLE_NAME}"
)
QUERY_METADATA_TABLE = environ.get(
    'SA_QUERY_METADATA_TABLE',
    f"{RESULTS_SCHEMA}.{QUERY_METADATA_TABLE_NAME}"
)
RUN_METADATA_TABLE = environ.get(
    'SA_METADATA_RUN_TABLE',
    f"{RESULTS_SCHEMA}.{RUN_METADATA_TABLE_NAME}"
)
DC_METADATA_TABLE = environ.get(
    'SA_METADATA_CONNECTOR_TABLE',
    f"{RESULTS_SCHEMA}.{DC_METADATA_TABLE_NAME}"
)
