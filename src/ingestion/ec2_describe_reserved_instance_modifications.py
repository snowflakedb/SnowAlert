...
INSTANCES_TABLE = os.environ['EC2_RESERVED_INSTANCE_MODIFICATIONS_LIST_SNOWFLAKE_TABLE_IDENTIFIER']
...
def get_data_worker(account):
    try:
        ...
        log.info(f"ec2_describe_reserved_instance_modifications: account: {account[1]} instances: {len(instance_list)}")
...
def main():
    ...
    if last_time is None or (current_time - last_time).total_seconds() > 86400:
        ...
        log.info("ec2_describe_reserved_instance_modifications: It's not time yet!")
