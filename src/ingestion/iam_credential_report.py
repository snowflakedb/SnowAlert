...
def get_data_worker(account):
    try:
        ...
        reports_json = [json.dumps({**report, "account_id": account[0]}) for report in reports]
        print(f"iam_credential_report: account: {account[1]} credentials: {len(reports_json)}")
...
def get_data(accounts_list):
    ...
    print(f"iam_credential_report: start: {start} end: {end} total: {(end - start).total_seconds()}")
